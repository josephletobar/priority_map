...
def load_graph_db(db_path):
    ...
    try:
        ...
        model_edges = []
        if _table_exists(cursor, "model_edges"):
            _required_columns(
                cursor,
                "model_edges",
                ["source_id", "target_id", "text", "created_by"],
            )
            cursor.execute(
                """
                SELECT source_id, target_id, text, created_by
                FROM model_edges
                ORDER BY source_id, target_id, text
                """
            )
            model_edges = [
                {
                ...
                for source_id, target_id, text, created_by in cursor.fetchall()
            ]
...
- model_edges are freeform model-written relationships.
...
say what is missing. Do not invent detections, coordinates, or relationships that
...
def run_chat(db_path):
    ...
    graph_data = load_graph_db(db_path)

    print("\nGraph DB chat started. Type 'quit' or 'exit' to leave.")
    print(
        f"Loaded {len(graph_data['nodes'])} node(s), "
        f"{len(graph_data['edges'])} spatial edge(s), and "
        f"{len(graph_data['model_edges'])} model edge(s)."
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Chat with an LLM about a saved priority-map graph.db file."
    )
